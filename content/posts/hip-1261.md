...
- **Unreadable transactions** – punitive fee charged to the node  
- **Invalid transactions** – network fee charged to the node  
- **Unhandled transactions** – node and network fees charged  
- **Failed transactions** – full fee charged  

This ensures resource usage is compensated regardless of success or failure.

### Fee Configuration and Estimation

The new fee schedule is stored as a JSON configuration in system file `0.0.113`.

HIP-1261 also introduces a mirror node API for fee estimation. This allows tools and SDKs to compute expected fees without submitting transactions, improving developer experience and transparency.
