 ...
 *
 * The star and call counts are live — `repository_stats.json` and
 * `community_calls.json` are regenerated by `pnpm sync:data` on every dev run
 * and build. The repository count is not: it counts the hand-curated
 * `reposData.repos` list above, and the star sum covers the separate repo list
 * hardcoded in `src/scripts/sync-repo-stats.mjs`. The two lists match today
 * and a test in `__tests__/homePageData.test.ts` fails the build if they
 * drift, so keep them in step when adding or removing a repository.
 */
