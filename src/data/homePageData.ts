...
export const whatIsHieroData = {
  ...
  text: "Hiero, a [Linux Foundation Decentralized Trust](https://www.lfdecentralizedtrust.org/) project, is an open-source, vendor-neutral distributed ledger technology. Hiero is used to build the [Hedera](https://hedera.com/) public ledger.",
  points: [
    {
...
/**
 * Declared last rather than first: the hero's colophon counts the repositories
 * and community calls defined above, so it has to be evaluated after them.
 *
 * Both counts are live. `repository_stats.json` and `community_calls.json` are
 * regenerated by `pnpm sync:data` on every dev run and build, so the numbers on
 * the hero move with the project instead of drifting into a stale claim.
 */
const trackedRepositoryCount = reposData.repos.length;

const trackedStarCount = Object.values(repositoryStats).reduce(
...
export const heroData = {
  ...
  heading: "Hiero",
  lede: "Open-source, vendor-neutral distributed ledger technology, developed in the open and used to build the Hedera public network.",
  actions: [
